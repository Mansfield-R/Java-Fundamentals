...
public class exercise22_PerfectNumber {
    ...
    public static boolean isPerfectNumber(int number) {
        ...
        // Need to keep track of original number since we are changing it in while loop:
        int totalOfDivisors = 0;
        int originalNumberComparator = number;
        ...
        if (number < 1) {
            ...
            int counter = 1;
            // Use less than instead of less than equal for Proper positive:
            while (counter < number) {
                // If remainder is 0 add that divisor to the divisorTotal and then keep counting
                if(number % counter == 0) {
                    totalOfDivisors += counter;
                }
                counter++;
            }
            // Test if the total of the divisors minus the number itself add up to number:
            if (totalOfDivisors == originalNumberComparator) {
                return true;
            } else {
                return false;
            }
